...
func main() {
	...
	for n := 1; n < len(os.Args); n++ {
		if 0 == strings.Compare(os.Args[n], "-port") {
			...
			n = n + 1
		} else if 0 == strings.Compare(os.Args[n], "-host") {
			host = os.Args[n+1]
	...
	var result ezmqx.EZMQXErrorCode = ezmqx.EZMQX_UNKNOWN_STATE
	if true == isStandAlone {
		if 0 == len(tnsAddr) {
			...
			result = configInstance.StartStandAloneMode(host, true, tnsAddress)
		}
		if result != ezmqx.EZMQX_OK {
