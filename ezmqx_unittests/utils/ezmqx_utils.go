...
var RUNNING_APP_INFO_RESPONSE = `{ "description": "services:  system-provisioning-director:    container_name: system-provisioning-director   image: system-provisioning-director/ubuntu_x86_64    labels:   - traefik.frontend.rule=PathPrefixStrip:/provisioning-service    - traefik.port=48198    network_mode: proxy    ports:    - 4000:4000    volumes:    - /system-provisioning-director/data/db:/data/dbversion: \"2\"", "images": [{ "name": "system-provisioning-director/ubuntu_x86_64" }], "services": [{ "cid": "` + hostName + `", "name": "system-provisioning-director", "ports": [{ "IP": "0.0.0.0", "PrivatePort": 4000, "PublicPort": 4000, "Type": "tcp" }], "state": { "exitcode": "0", "status": "running" } }], "state": "running" }`
